...
namespace hookapi
{
	...
	private:
		...
		//	m_origin = (byte*)VirtualAlloc(nullptr, MIN_HOOK_SIZE + 8, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
			...
			{
				memcpy(m_origin, m_api, m_saved_size);										//origin->Execute the first several instruction of the origin api
	...
	//	if(nullptr != p_context) return *p_context;
		...
		if(GetEnvironmentVariable(key.c_str(), env, _countof(env)))
		{
		...
		SetEnvironmentVariable(key.c_str(), env);
		return p_hookmap;
	}
